...
echo -e "  [+] Compilation des binaires release..."
cargo build --release
...
if [ -f "$HOME/.zshrc" ]; then
    ...
    if ! grep -q "skipper-env.sh" "$HOME/.zshrc"; then
        ...
        echo "  [OK] Intégration ajoutée à ~/.zshrc"
    fi
fi
...
if [ -f "$HOME/.bashrc" ]; then
    ...
    if ! grep -q "skipper-env.sh" "$HOME/.bashrc"; then
        ...
        echo "  [OK] Intégration ajoutée à ~/.bashrc"
    fi
fi
...
if [ -d "$HOME/.config/fish" ]; then
    ...
    echo "  [OK] Complétion Fish installée dans $FISH_COMP_DIR/skipper.fish"
fi

echo -e "${CYAN}──────────────────────────────────────────────────${RESET}"
echo -e "${BOLD}${GREEN}[OK] Skipper 360 a été déployé avec succès sur votre ordinateur.${RESET}"
echo -e "Pour appliquer immédiatement, faites : ${YELLOW}source ~/.zshrc${RESET} (ou relancez le terminal)"
